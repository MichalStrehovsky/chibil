...
public class MsvcNameMangler : NameMangler
{
    private readonly TypeSystem _types;
    private int _anonGlobalCounter;

    public MsvcNameMangler(TypeSystem types, string tuHash)
        : base(tuHash)
    {
        _types = types;
    }
    ...
    /// </summary>
    public override string MangleFunctionName(Obj fn)
    {
        CType funcTy = fn.Ty;
        ...
        string name = MangleFunctionBaseName(fn);
        var sb = new StringBuilder();
        sb.Append($"?{name}@@$$J0Y{cc}");

        var mangler = new Mangler(_types, fn.Name); // function name = slot 0
    ...
    public override string MangleUnmanagedEntryPointName(Obj fn)
        => $"__unep@{MangleFunctionName(fn)}";

    /// <summary>
    /// Generate array TypeDef name: $ArrayType$$$BY(ndims)(bounds)(elemtype)
    /// </summary>
    public override string MangleArrayTypeName(CType ty)
    {
    ...
    public override string MangleStaticLocalName(Obj var)
        => $"?A0x{_tuHash}.{var.Name}";

    public override string MangleStaticGlobalName(string name) =>
        $"?A0x{_tuHash}.{name}";
    ...
    private readonly struct Mangler
    {
        ...
        public Mangler(TypeSystem ts, string nameBackref)
        {
            _types = ts;
        ...
        public Mangler(TypeSystem ts, bool useArgBackrefs)
        {
            _types = ts;
            _nameBackRefs = null;
        ...
        /// </summary>
        public void MangleArgType(StringBuilder sb, CType ty)
        {
            ...
            var nestedMangler = new Mangler(_types, useArgBackrefs: true);
        ...
        public void MangleType(StringBuilder sb, CType ty, bool isReturn)
        {
