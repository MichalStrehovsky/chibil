...
pal.c is the platform abstraction layer. It lets DOOM talk to the world (read files, push pixels, handle input). The current pal.c only works on Windows. The rules are the same: cannot include Windows.h, we likely wouldn't be able to parse it (haven't tried).

doom.c and pal.c support two harness ifdefs:
