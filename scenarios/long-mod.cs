...
public class LongModTest
{
    ...
    public void Emit(Machine machine)
    {
        ...
        string emittedDir = Path.Combine(AppContext.BaseDirectory, "emitted", "long-mod", refDir);
        Directory.CreateDirectory(emittedDir);
        File.WriteAllBytes(Path.Combine(emittedDir, "long-mod.obj"), emitted);
    ...
    static byte[] EmitObj(Machine machine)
    {
        ...
        var alSig = new BlobBuilder();
        {
            ...
            // Return: modopt(CallConvCdecl) modopt(IsLong) int32
        ...
        var auSig = new BlobBuilder();
        {
            ...
            enc.Parameters(2, out var retEnc, out var parEnc);
            // Return: modopt(CallConvCdecl) modopt(IsLong) uint32
            var retType = ClrIjw.WriteCdeclModOpt(retEnc, callConvCdeclRef);
            retType.Builder.WriteByte((byte)SignatureTypeCode.OptionalModifier);
            retType.Builder.WriteCompressedInteger(CodedIndex.TypeDefOrRefOrSpec(isLongRef));
        ...
        // ─── Emit IL for add_long ─────────────────────────────────────────
        {
            ...
            bodyEncoder.AddMethodBody(addLongMethod, "?add_long@@$$J0YAJJJ@Z", enc,
        ...
        // ─── Emit IL for add_ulong ────────────────────────────────────────
        {
            ...
            bodyEncoder.AddMethodBody(addUlongMethod, "?add_ulong@@$$J0YAKKK@Z", enc,
