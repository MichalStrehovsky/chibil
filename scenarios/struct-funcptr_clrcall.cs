...
public class StructFuncptrClrcallTest
{
    [Theory]
    ...
    public void Emit(Machine machine)
    {
        ...
        string emittedDir = Path.Combine(AppContext.BaseDirectory, "emitted", "struct-funcptr_clrcall", refDir);
        Directory.CreateDirectory(emittedDir);
        File.WriteAllBytes(Path.Combine(emittedDir, "struct-funcptr_clrcall.obj"), emitted);

        byte[] reference = File.ReadAllBytes(
            Path.Combine(AppContext.BaseDirectory, "reference", "struct-funcptr_clrcall", refDir, "struct-funcptr_clrcall.obj"));
    ...
    static byte[] EmitObj(Machine machine)
    {
        ...
        // ─── TypeDef #2: _Handler ─────────────────────────────────────────
        // x86: size=8 (4 bytes fnptr + 4 bytes int), arm64: size=16 (8+4+padding)
        ...
        // ─── StandaloneSignature for calli: DEFAULT I4(I4) ────────────────
        ...
        // ─── Module ───────────────────────────────────────────────────────
        md.AddModule(0,
            md.GetOrAddString("struct-funcptr_clrcall.obj"),
        ...
        string objPath = "struct-funcptr_clrcall.obj";
        ...
        string sourceFile = Path.Combine(AppContext.BaseDirectory, "struct-funcptr_clrcall.c");
        ...
        // ─── Emit IL for invoke ───────────────────────────────────────────
        {
            ...
            {
                // arm64/x64: h->value at offset 8, h->callback at offset 0
                enc.MarkLineNumber(cvFile, 13);
                enc.OpCode(ILOpCode.Ldarg_0);             // IL_0000
            ...
            bodyEncoder.AddMethodBody(invokeMethod, $"?invoke@@$$J0YMHP{e}AU_Handler@@@Z", enc,
                maxStack: 2, localVariablesSignature: doubleLocalsSigHandle, attributes: 0,
                debugName: "invoke");
        }
        ...
        // ─── IJW machinery for double_it and invoke only ─────────────────
